...
class EmbeddingTransformationLayer(nn.Module):

    def __init__(self, hparams):
        super(EmbeddingTransformationLayer, self).__init__()
        self.linear = nn.Linear(hparams.speaker_embedding_dim,
                                hparams.speaker_embedding_output_dim)
        self.dropout = hparams.speaker_embedding_transformation_dropout

    def forward(self,x):
        x = self.linear(x)
        x = F.dropout(F.softsign(x), self.dropout)
        return x
...
class Decoder(nn.Module):
    def __init__(self, hparams):
        ...
        self.embeddings_dim = hparams.encoder_embedding_dim + hparams.speaker_embedding_output_dim
        ...
        self.attention_rnn = nn.LSTMCell(
            hparams.prenet_dim + self.embeddings_dim,
            hparams.attention_rnn_dim)

        self.attention_layer = Attention(
            hparams.attention_rnn_dim, self.embeddings_dim,
        ...
        self.linear_projection = LinearNorm(
            hparams.decoder_rnn_dim + self.embeddings_dim,
            hparams.n_mel_channels * hparams.n_frames_per_step)

        self.gate_layer = LinearNorm(
            hparams.decoder_rnn_dim + self.embeddings_dim, 1,
    ...
    def forward(self, memory, decoder_inputs, memory_lengths):
        ...
        self.initialize_decoder_states(
            memory, mask=1-get_mask_from_lengths(memory_lengths,self.use_cpu))
...
class Tacotron2SimpleCat(nn.Module):
    def __init__(self, hparams):
        super(Tacotron2SimpleCat, self).__init__()
        ...
        self.speaker_transformation = EmbeddingTransformationLayer(hparams)
    ...
    def parse_output(self, outputs, output_lengths=None):
        if self.mask_padding and output_lengths is not None:
            mask = 1-get_mask_from_lengths(output_lengths,self.use_cpu)
